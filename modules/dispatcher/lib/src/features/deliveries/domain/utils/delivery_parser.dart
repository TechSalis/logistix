...
/// A parsed delivery with a 0–1 confidence score and any warnings from the engine.
class ParsedDelivery {
  ...
  final DeliveryCreateInput delivery;
  final double confidence;
  final List<String> warnings;
}

/// A chunk of text that failed to produce a valid delivery.
class ParseFailure {
  const ParseFailure({
...
class LocalParseResult {
  const LocalParseResult({required this.deliveries, required this.failures});

  final List<ParsedDelivery> deliveries;
  ...
  double get overallConfidence {
    ...
    return deliveries.fold<double>(0, (s, o) => s + o.confidence) / deliveries.length;
  }
...
/// Intent-aware, fuzzy-matching delivery parser.
///
...
class DeliveryParser {
  static Future<LocalParseResult> parse(String text) async {
    if (text.trim().isEmpty) {
      return const LocalParseResult(deliveries: [], failures: []);
    }
  ...
  static Map<String, dynamic> _isolateEntry(String text) {
    try {
      ...
      return {'deliveries': <dynamic>[], 'failures': <dynamic>[]};
    }
  }

  static LocalParseResult _deserialise(Map<String, dynamic> raw) {
    final deliveries = (raw['deliveries'] as List).map((o) {
      final m = o as Map<String, dynamic>;
      return ParsedDelivery(
        delivery: DeliveryCreateInput.fromJson(m['delivery'] as Map<String, dynamic>),
    ...
    return LocalParseResult(deliveries: deliveries, failures: failures);
  }
}
...
const _kDesc    = ['description', 'item', 'package', 'parcel', 'content', 'goods', 'note', 'remark', 'delivery'];
...
class _Engine {
  ...
  Map<String, dynamic> run() {
    ...
    for (final chunk in chunks) {
      ...
      if (result == null) {
        failures.add({'rawInput': chunk, 'reason': 'No recognisable delivery data', 'confidence': 0.0});
  ...
  List<String> _split(String text) {
    ...
    // Numbered entry markers ("1.", "1:", "Delivery 2 -", "Delivery 3:")
    final numbered = RegExp(
      r'(?:^|\n)(?:\*\*)?(?:\d+[.)]\s*|Delivery\s*\d+[:\-\s]|Delivery\s*\d+[:\-\s]|Customer\s*\d+[:\-\s])(?:\*\*)?',
      caseSensitive: false,
    );
    ...
    // Multiple pickup/dropoff label pairs (≥4 signals multi-delivery block)
  ...
  Map<String, dynamic>? _parseChunk(String chunk) {
    ...
    final delivery = DeliveryCreateInput(
