...
class _LineClassifier {
  ...
  _IntentResult classify(String line) {
    ...
    if (_kvLabelRx.hasMatch(line)) {
      return _IntentResult(_LineIntent.phraseHint, 0.7);
    }
    ...
    for (final phrase in _kDropoffOwnerPhrases) {
      if (lower.contains(phrase)) {
        return _IntentResult(_LineIntent.phraseHint, 0.8, roleHint: _Role.dropoff);
      }
    }
    for (final phrase in _kPickupOwnerPhrases) {
      if (lower.contains(phrase)) {
        return _IntentResult(_LineIntent.phraseHint, 0.8, roleHint: _Role.pickup);
      }
    }
    ...
    if (addrScore >= 0.28) {
      var role = _roleFromContext(lower);
    ...
    if (wordCount >= 2 && line.length > 5) {
      return _IntentResult(_LineIntent.description, 0.4);
    }

    return _IntentResult(_LineIntent.noise, 0.1);
  }
