...
import 'package:logistix/features/map/infrastructure/repository/google_places_search_location_service.dart';
import 'package:logistix/features/map/domain/repository/search_location_service.dart';
import 'package:logistix/features/map/infrastructure/datasources/google_places_datasource.dart';
...
final _searchLocationProvider = Provider.autoDispose<SearchLocationService>(
  (ref) => GooglePlacesSearchLocationServiceImpl(ref.watch(placesApi)),
);
...
class LocationSearchNotifier
  ...
  Future<void> getPlaceData(Address address) async {
    final place = await ref.read(_searchLocationProvider).place(address);
