...
class AuthSession {
  ...
  const AuthSession.unauthenticated() : status = AuthStatus.unauthenticated, user = null;
  const AuthSession.authenticated(User user) : status = AuthStatus.authenticated, user = user;
  const AuthSession.onboarding(User user) : status = AuthStatus.onboarding, user = user;
...
abstract class AuthStatusRepository {
  ...
  /// Updates the status to [AuthStatus.onboarding].
  void setOnboarding(User user);
...
class AuthStatusRepositoryImpl implements AuthStatusRepository {
  ...
  void _update(AuthSession session) {
    if (_currentSession.status == session.status && _currentSession.user?.id == session.user?.id) return;
